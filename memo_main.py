...
######################################             Тестові дані:         #############################################
def testlist():
    frm = Form('Какой босс появляется в начале игры?', 'B) Глаз Ктулху',
               'A) Скелетрон', 'C) Древний дух', 'D) Механический червь')
    questions_listmodel.form_list.append(frm)

    frm = Form('Что нужно для создания "Стены"?', 'A) Дерево',
               'B) Камень', 'C) Земля','D) Крафтовые блоки')
    questions_listmodel.form_list.append(frm)

    frm = Form('Какой тип оружия можно создать из жемчуга?', 'A) Меч',
               'B) Лук', 'C) Посох', 'D) Ловушки')
    questions_listmodel.form_list.append(frm)

    frm = Form('Какой элемент является основой для создания многих магических предметов?', 'A) Кристаллы',
               'B) Пыльца', 'C) Жемчуг','D) Сила ветра')
    questions_listmodel.form_list.append(frm)
    frm = Form('Какой биом не встречается в игре?', 'C) Ледяная равнина',
               'A) Лес', 'B) Пустыня', 'D) Облачный биом')
    questions_listmodel.form_list.append(frm)
    frm = Form('Какой NPC продает семена и растения?', 'A) Садовник',
               'B) Древний маг', 'C) Путешественник', 'D) Бард')
    questions_listmodel.form_list.append(frm)

    frm = Form('Что можно создать из "Золотых слитков"?', 'C) Удочка',
               'A) Серебряный меч', 'B) Золотая броня', 'D) Лук')
    questions_listmodel.form_list.append(frm)

    frm = Form('Какой биом содержит большое количество лавы?', 'C) Ад',
               'A) Пустыня', 'B) Джунгли', 'D) Снежные равнины')
    questions_listmodel.form_list.append(frm)

    frm = Form('Какой из этих предметов нужен для вызова Глаза Ктулху?', 'D) Око',
               'A) Призывной шар', 'B) Звезда смерти', 'C) Лунный механизм')
    questions_listmodel.form_list.append(frm)
    frm = Form('Какой ресурс используется для крафта "Серебряной брони"?', 'A) Серебро',
               'B) Железо', 'C) Уголь', 'D) Сталь')
    questions_listmodel.form_list.append(frm)
